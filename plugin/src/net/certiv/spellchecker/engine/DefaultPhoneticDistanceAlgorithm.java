...
public final class DefaultPhoneticDistanceAlgorithm implements IPhoneticDistanceAlgorithm {
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.IPhoneticDistanceAlgorithm#getDistance(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public int getDistance(final String from, final String to) {
		...
		for (int row = 1; row < rows; row++) {
			...
			for (int column = 1; column < columns; column++) {
				...
				if (Character.toLowerCase(source) == Character.toLowerCase(target)) change = COST_CASE + diagonal;
				...
				if (swap < minimum) minimum = swap;

				remove = metric[row][column - 1];
				if (COST_REMOVE + remove < minimum) minimum = COST_REMOVE + remove;

				insert = metric[row - 1][column];
				if (COST_INSERT + insert < minimum) minimum = COST_INSERT + insert;
				if (change < minimum) minimum = change;
