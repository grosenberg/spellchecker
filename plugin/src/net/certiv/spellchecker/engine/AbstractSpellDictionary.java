...
import net.certiv.spellchecker.etc.BasicElementLabels;
...
public abstract class AbstractSpellDictionary implements ISpellDictionary {
	 ...
	 *
	 * @param hash The hash to retrieve the candidates of
	...
	protected final Set<RankedWordProposal> getCandidates(final String word, final boolean sentence,
		...
		for (String hash2 : hashs) {
			...
			int candidateSize = Math.min(500, candidateList.size()); // see
																	 // https://bugs.eclipse.org/bugs/show_bug.cgi?id=195357
	...
	/**
	 * Returns all approximations that have a phonetic hash with smallest possible
	...
	protected final void getCandidates(final String word, final boolean sentence,
		...
		for (byte[] element : candidateList) {
			...
			if (distance <= minimum) {
				...
				if (distance < minimum) matches.clear();
	 ...
	 *
	 * @throws MalformedURLException if the URL could not be retrieved
	 ...
	 *
	 * @param word The word to hash in the dictionary
	 */
	...
	public boolean isCorrect(String word) {
		...
		if (candidates == null) return false;
			...
			if (candidate.equals(word) || candidate.equals(word.toLowerCase())) return true;
			return false;
		}
		@SuppressWarnings("unchecked")
		final ArrayList<byte[]> candidateList = (ArrayList<byte[]>) candidates;
	...
	protected String stripNonLetters(String word) {
		if (!fIsStrippingNonLetters) return word;

		int i = 0;
		int j = word.length() - 1;
		while (i <= j && !Character.isLetter(word.charAt(i)))
			i++;
		if (i > j) return ""; //$NON-NLS-1$
		...
		if (url != null) {
			...
			try {
				...
				if (stream != null) {
					...
					try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream, decoder))) {
						boolean doRead = true;
						while (doRead) {
							try {
								...
								String message = Messages.format(
										JavaUIMessages.AbstractSpellingDictionary_encodingError, new String[] { word,
												decoder.replacement(), BasicElementLabels.getURLPart(url.toString()) });
								IStatus status = new Status(IStatus.ERROR, Activator.ID_PLUGIN, IStatus.OK, message,
										ex);
				...
				if (urlString.equals(lowercaseUrlString)) Activator.log(ex);
				else try {
	...
	private void compact() {
		...
		while (iter.hasNext()) {
			...
			if (element instanceof ArrayList) ((ArrayList<?>) element).trimToSize();
		}
	}
	 ...
	 *
	 * @param algorithm The phonetic distance algorithm
	 */
	 ...
	 *
	 * @param provider The phonetic hash provider
	 */
	...
	protected String getEncoding() {
		String encoding = Activator.getDefault().getPreferenceStore()
				.getString(PreferenceConstants.SPELLING_USER_DICTIONARY_ENCODING);
		if (encoding == null || encoding.length() == 0) encoding = ResourcesPlugin.getEncoding();
		return encoding;
	}
