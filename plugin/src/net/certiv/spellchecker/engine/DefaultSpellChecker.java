...
public class DefaultSpellChecker implements ISpellChecker {
	...
	protected static boolean isDigits(final String word) {
		...
		for (int index = 0; index < word.length(); index++) {
			...
			if (Character.isDigit(word.charAt(index))) return true;
		}
		return false;
	}
	...
	protected static boolean isUrl(final String word) {
		...
		for (String element : URL_PREFIXES) {
			...
			if (word.startsWith(element)) return true;
		}
		return false;
	}

	/**
	 * The dictionaries to use for spell checking. Synchronized to avoid concurrent
	 * modifications.
	 */
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.ISpellChecker#addDictionary(org.eclipse.spelling.done.
	 * ISpellDictionary)
	 */
	@Override
	...
	public boolean acceptsWords() {
		...
		for (final Iterator<ISpellDictionary> iterator = copy.iterator(); iterator.hasNext();) {
			...
			if (dictionary.acceptsWords()) return true;
		}
		return false;
	}

	/*
	 * @see net.certiv.spellchecker.engine.ISpellChecker#addWord(java.lang.String)
	 */
	@Override
	public void addWord(final String word) {
		...
		for (ISpellDictionary dictionary : copy) {
			if (dictionary.acceptsWords()) dictionary.addWord(addable);
		}
	...
	/*
	 * @see
	 * org.eclipse.jdt.ui.text.spelling.engine.ISpellChecker#checkWord(java.lang.String)
	 */
	@Override
	...
	/*
	 * @see org.eclipse.spelling.done.ISpellChecker#execute(org.eclipse.spelling.
				...
				if (!fIgnored.contains(word)) {
					...
					if (!isCorrect(word)) {
						...
						if (!ignoreSentence && starts && Character.isLowerCase(word.charAt(0))) {
							listener.handle(
									new SpellEvent(this, word, iterator.getBegin(), iterator.getEnd(), true, true));
							problemCount++;
						}
					}
				}
	...
	/*
	 * @see net.certiv.spellchecker.engine.ISpellChecker#ignoreWord(java.lang.String)
	 */
	@Override
	...
	/*
	 * @see net.certiv.spellchecker.engine.ISpellChecker#isCorrect(java.lang.String)
	 */
	@Override
	public final boolean isCorrect(final String word) {
		...
		if (fIgnored.contains(word.toLowerCase())) return true;
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.ISpellChecker#removeDictionary(org.eclipse.spelling.done.
	 * ISpellDictionary)
	 */
	@Override
	...
	/*
	 * @see net.certiv.spellchecker.engine.ISpellChecker#getLocale()
	 *
	 * @since 3.3
	 */
	@Override
