...
public final class DefaultPhoneticHashProvider implements IPhoneticHashProvider {
	...
	/**
	 * Test whether the specified string contains one of the candidates in the list.
	 *
	 * @param candidates Array of candidates to check
	 * @param token The token to check for occurrences of the candidates
	 * @param offset The offset where to begin checking in the string
	 * @param length The length of the range in the string to check
	 * @return {@code true} iff the string contains one of the candidates, {@code false}
	 *             otherwise.
	 */
	protected static boolean hasOneOf(final String[] candidates, final char[] token, final int offset,
			final int length) {
		...
		for (String candidate : candidates) {
			...
			if (candidate.equals(checkable)) return true;
		}
		return false;
	}

	/**
	 * Test whether the specified token contains one of the candidates in the list.
	 *
	 * @param candidates Array of candidates to check
	 * @param token The token to check for occurrences of the candidates
	 * @return {@code true} iff the string contains one of the candidates, {@code false}
	...
	protected static boolean hasOneOf(final String[] candidates, final String token) {
		...
		for (String candidate : candidates) {
			...
			if (token.indexOf(candidate) >= 0) return true;
		}
		return false;
	}

	/**
	 * Tests whether the specified token contains a vowel at the specified offset.
	 *
	 * @param token The token to check for a vowel
	 * @param offset The offset where to begin checking in the token
	 * @param length The length of the range in the token to check
	 * @return {@code true} iff the token contains a vowel, {@code false} otherwise.
	 */
	...
	public String getHash(final String word) {
		...
		int offset = 0;
		if (hasOneOf(meta26, hashable, 0, 2)) offset += 1;
		...
		while (offset < hashable.length) {
			...
			switch (hashable[offset]) {
				...
				case 'U':
				case 'Y':
					if (offset == 0) buffer.append('A');
					offset += 1;
					break;
				case 'B':
					buffer.append('P');
					if (hashable[offset + 1] == 'B') offset += 2;
					else offset += 1;
					break;
				case 'C':
					...
					if (hasOneOf(meta05, hashable, offset, 2)) {
						...
						if (hasOneOf(meta10, hashable, 0, 4) || hasOneOf(meta11, hashable, 0, 3)
							...
							if (offset > 0) {
								if (hasOneOf(meta16, hashable, 0, 2)) buffer.append('K');
					...
					buffer.append('K');
					if (hasOneOf(meta27, hashable, offset + 1, 2)) offset += 3;
				...
				case 'D':
					...
					break;
				case 'F':
					if (hashable[offset + 1] == 'F') offset += 2;
					else offset += 1;
					buffer.append('F');
					break;
				case 'G':
					if (hashable[offset + 1] == 'H') {
						...
						if (offset < 3) {
							if (offset == 0) {
								if (hashable[offset + 2] == 'I') buffer.append('J');
						...
						if ((offset > 1) && hasOneOf(meta33, hashable, offset - 2, 1)
							...
							if ((offset > 2) && (hashable[offset - 1] == 'U')
								...
								if ((offset > 0) && (hashable[offset - 1] != 'I')) buffer.append('K');
							}
							offset += 2;
							break;
						}
					...
					if (hashable[offset + 1] == 'G') offset += 2;
					else offset += 1;
					buffer.append('K');
					break;
				case 'H':
				...
				case 'J':
					...
					if (hashable[offset + 1] == 'J') offset += 2;
					else offset += 1;
					break;
				case 'K':
					if (hashable[offset + 1] == 'K') offset += 2;
					else offset += 1;
					buffer.append('K');
					break;
				case 'L':
				...
				case 'M':
					...
					break;
				case 'N':
					if (hashable[offset + 1] == 'N') offset += 2;
					else offset += 1;
					buffer.append('N');
					break;
				case '\u00D1':
				...
				case 'P':
					...
					if (hasOneOf(meta62, hashable, offset + 1, 1)) offset += 2;
					else offset += 1;
					buffer.append('P');
					break;
				case 'Q':
					if (hashable[offset + 1] == 'Q') offset += 2;
					else offset += 1;
					buffer.append('K');
					break;
				case 'R':
					if (((offset != (hashable.length - 1)) || has95 || !hasOneOf(meta63, hashable, offset - 2, 2) || hasOneOf(meta64, hashable, offset - 4, 2)))
						buffer.append('R');
					if (hashable[offset + 1] == 'R') offset += 2;
					else offset += 1;
					break;
				case 'S':
					...
					if (hasOneOf(meta67, hashable, offset, 2)) {
						if (hasOneOf(meta68, hashable, offset + 1, 4)) buffer.append('S');
						else buffer.append('X');
						offset += 2;
						break;
					}
					...
					if (((offset == 0) && hasOneOf(meta71, hashable, offset + 1, 1))
						...
						buffer.append('S');
						if (hasOneOf(meta73, hashable, offset + 1, 1)) offset += 2;
						else offset += 1;
						break;
					}
					if (hasOneOf(meta74, hashable, offset, 2)) {
						if (hashable[offset + 2] == 'H') if (hasOneOf(meta75, hashable, offset + 3, 2)) {
					...
					if (((offset != (hashable.length - 1)) || !hasOneOf(meta78, hashable, offset - 2, 2)))
						buffer.append('S');
					if (hasOneOf(meta79, hashable, offset + 1, 1)) offset += 2;
					else offset += 1;
					break;
				case 'T':
					...
					break;
				case 'V':
					if (hashable[offset + 1] == 'V') offset += 2;
					else offset += 1;
					buffer.append('F');
					break;
				case 'W':
					...
					if ((offset == 0) && (hasVowel(hashable, offset + 1, hashable.length)
							|| hasOneOf(meta89, hashable, offset, 2))) {
						buffer.append('A');
					}
				...
				case 'X':
					if (((offset != (hashable.length - 1)) || (!hasOneOf(meta93, hashable, offset - 3, 3) && !hasOneOf(meta94, hashable, offset - 2, 2))))
						buffer.append("KS"); //$NON-NLS-1$
					if (hasOneOf(meta49, hashable, offset + 1, 1)) offset += 2;
					else offset += 1;
					break;
				case 'Z':
					if (hashable[offset + 1] == 'H') {
					...
					if (hashable[offset + 1] == 'Z') offset += 2;
					else offset += 1;
					break;
				default:
					offset += 1;
