...
public class StringMatcher {
	 ...
	 * character. Literal '*' and '?' characters must be escaped in the pattern e.g., "\*"
	 * means literal "*", etc. Escaping any other character (including the escape
	 * character itself), just results in that character in the pattern. e.g., "\a" means
	 * "a" and "\\" means "\" If invoking the StringMatcher with string literals in Java,
	 * don't forget escape characters are represented by "\\".
	 *
	 ...
	 *             (inclusive) and ending positions (exclusive) of the first occurrence of
	 *             the pattern in the specified range of the text; return null if not
	 *             found or subtext is empty (start==end). A pair of zeros is returned if
	 *             pattern is empty string Note that for pattern like "*abc*" with leading
	 *             and trailing stars, position of "abc" is returned. For a pattern
	...
	public boolean match(String text, int start, int end) {
		...
		int segCount = fSegments.length;
		if (segCount == 0 && (fHasLeadingStar || fHasTrailingStar))  // pattern contains
																	  // only '*'(s)
			return true;
