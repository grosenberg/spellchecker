...
import net.certiv.spellchecker.engine.ISpellDictionary;
...
public class SpellCheckEngine implements ISpellCheckEngine, IPropertyChangeListener {
	...
	private static Set<Locale> getLocalesWithInstalledDictionaries(URL location) {
		...
		try {
			...
			fileNames = file.list();
			if (fileNames == null) return Collections.emptySet();
	...
	public static Set<Locale> getLocalesWithInstalledDictionaries() {
		if (fgLocalesWithInstalledDictionaries != null) return fgLocalesWithInstalledDictionaries;

		Enumeration<URL> locations;
		try {
			locations = getDictionaryLocations();
			if (locations == null) return fgLocalesWithInstalledDictionaries = Collections.emptySet();
		} catch (IOException ex) {
			Activator.log(ex);
	...
	public ISpellDictionary findDictionary(Locale locale) {
		...
		while (iter.hasNext()) {
			...
			if (dictLocale.getLanguage().equals(language)) return entry.getValue();
		}
	...
	public static Locale findClosestLocale(Locale locale) {
		if (locale == null || locale.toString().length() == 0) return locale;

		if (getLocalesWithInstalledDictionaries().contains(locale)) return locale;
		...
		if (getLocalesWithInstalledDictionaries().contains(defaultLocale)) return defaultLocale;
	...
	public static Enumeration<URL> getDictionaryLocations() throws IOException {
		...
		if (plugin != null) return plugin.getBundle().getResources("/" + DICTIONARY_LOCATION); //$NON-NLS-1$
		return null;
	}
	 ...
	 */
	public static synchronized final ISpellCheckEngine getInstance(IPreferenceStore store) {
		if (fgEngine == null) {
			fgEngine = new SpellCheckEngine(store);
		}
		return fgEngine;
	}

	public static synchronized final ISpellCheckEngine getInstance() {
		assert fgEngine != null;
		return fgEngine;
	}
	...
	private SpellCheckEngine(IPreferenceStore store) {
		...
		try {
			...
			while (locations != null && locations.hasMoreElements()) {
				...
				for (final Iterator<Locale> iterator = getLocalesWithInstalledDictionaries(location)
	...
	public synchronized final ISpellChecker getSpellChecker() throws IllegalStateException {
		...
		if (fChecker != null && fChecker.getLocale().equals(locale)) return fChecker;
		...
		if (dictionary != null) fChecker.addDictionary(dictionary);
	...
	public static Locale convertToLocale(String locale) {
		...
		if (locale.equals(defaultLocale.toString())) return defaultLocale;

		int length = locale.length();
		if (length >= 5) return new Locale(locale.substring(0, 2), locale.substring(3, 5));

		if (length == 2 && locale.indexOf('_') == -1) return new Locale(locale);

		if (length == 3 && locale.charAt(0) == '_') return new Locale("", locale.substring(1)); //$NON-NLS-1$
	...
	private synchronized void resetUserDictionary() {
		...
		if (variablesPlugin == null) return;
	...
	private synchronized void resetSpellChecker() {
		if (fChecker != null) {
			...
			if (dictionary != null) dictionary.unload();
		}
		fChecker = null;
	}
