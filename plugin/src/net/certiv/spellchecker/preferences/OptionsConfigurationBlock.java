...
import net.certiv.spellchecker.etc.CoreUtility;
import net.certiv.spellchecker.etc.IStatusChangeListener;
import net.certiv.spellchecker.etc.StringMatcher;
...
public abstract class OptionsConfigurationBlock {
	...
	protected static class PreferenceTreeNode {
		...
		/**
		 * Label text of the preference which is used for filtering. This text does not
		 * contain {@code &} which is used to indicate mnemonics.
		 */
		private final String fLabel;

		/**
		 * The preference key or the local key to uniquely identify a node's corresponding
		 * UI control. Can be {@code null}.
		 */
		private final Key fKey;

		/**
		 * Tells whether all children should be shown even if just one child matches the
		 * filter.
		 */
		private final boolean fShowAllChildren;

		/**
		 * Tells whether this node's UI control is visible in the UI for the current
		 * filter text.
		 */
		 ...
		 * <p>
		 * The {@code label} and the {@code key} must not be {@code null} if the node has
		 * a corresponding UI control.
		 *
		 * @param label the label text
		 * @param key the key
		 * @param controlType the type of UI control.
		 * @param showAllChildren tells whether all children should be shown even if just
		...
		private void setVisible(boolean visible, boolean recursive) {
			...
			if (!recursive) return;
			if (fChildren != null) {
				for (PreferenceTreeNode fChild : fChildren) {
					fChild.setVisible(visible, recursive);
				}
			}
		}
	 ...
	 * <p>
	 * The tree consists of an optional description label, a filter text input box, and a
	 * scrolled area. The scrolled content contains all the UI controls which participate
	 * in filtering.
	 * <p>
	 * Supports '*' and '?' wildcards. A word in filter text preceded by '~' is used to
	 * filter on preference values, e.g. ~ignore or ~off. Supported filter formats are
	 * <ul>
	 * <li>pattern</li>
	 * <li>~valueFilter</li>
	...
	protected static class FilteredPreferenceTree {
		...
		/**
		 * Constructs a new instance of FilteredPreferenceTree according to the
		 * parameters.
		 *
		...
		private void createFilterBox() {
			...
			filterBox.addModifyListener(new ModifyListener() {
				...
				public void modifyText(ModifyEvent e) {
					...
					if (input != null && input.equalsIgnoreCase(fPrevFilterText)) return;
		...
		public PreferenceTreeNode addChild(PreferenceTreeNode parent, String label, Key key, int controlType,
				boolean showAllChildren) {
			parent = (parent == null) ? fRoot : parent;
			return parent.addChild(label, key, controlType, showAllChildren);
		}

		public PreferenceTreeNode addCheckBox(Composite parentComposite, String label, Key key, String[] values,
				int indent, PreferenceTreeNode parentNode, boolean showAllChildren) {
		...
		public PreferenceTreeNode addCheckBox(Composite parentComposite, String label, Key key, String[] values,
				int indent, PreferenceTreeNode parentNode) {
			return addCheckBox(parentComposite, label, key, values, indent, parentNode, true);
		}

		public PreferenceTreeNode addCheckBoxWithLink(Composite parentComposite, String label, Key key, String[] values,
				int indent, PreferenceTreeNode parentNode, boolean showAllChildren, int widthHint,
		...
		public PreferenceTreeNode addComboBox(Composite parentComposite, String label, Key key, String[] values,
				String[] valueLabels, int indent, PreferenceTreeNode parentNode, boolean showAllChildren) {
		...
		public PreferenceTreeNode addComboBox(Composite parentComposite, String label, Key key, String[] values,
				String[] valueLabels, int indent, PreferenceTreeNode parentNode) {
			return addComboBox(parentComposite, label, key, values, valueLabels, indent, parentNode, true);
		}

		public PreferenceTreeNode addTextField(Composite parentComposite, String label, Key key, int indent,
				int widthHint, PreferenceTreeNode parentNode) {
		...
		public PreferenceTreeNode addExpandableComposite(Composite parentComposite, String label, int nColumns, Key key,
				PreferenceTreeNode parentNode, boolean showAllChildren) {
		...
		public PreferenceTreeNode addLink(Composite parentComposite, String label, Key key,
				SelectionListener linkListener, int indent, int widthHint, PreferenceTreeNode parentNode) {
		...
		private boolean match(PreferenceTreeNode node, StringMatcher labelMatcher, StringMatcher valueMatcher) {
			...
			if (valueMatcher != null) {
				if (node.getControlType() == PreferenceTreeNode.COMBO) {
					...
					if (checked) {
						valueMatched = valueMatcher.match(PreferencesMessages.OptionsConfigurationBlock_On)
								|| valueMatcher.match(PreferencesMessages.OptionsConfigurationBlock_Enabled);
					} else {
						valueMatched = valueMatcher.match(PreferencesMessages.OptionsConfigurationBlock_Off)
								|| valueMatcher.match(PreferencesMessages.OptionsConfigurationBlock_Disabled);
					}
				}
			}
		...
		public boolean filter(PreferenceTreeNode node, StringMatcher labelMatcher, StringMatcher valueMatcher) {
			...
			if (children != null) {
				for (PreferenceTreeNode child : children) {
					visible |= filter(child, labelMatcher, valueMatcher);
				}
		...
		private StringMatcher createStringMatcher(String filterText) {
			filterText = filterText.trim();
			if (filterText.length() > 0) return new StringMatcher("*" + filterText + "*", true, false); //$NON-NLS-1$ //$NON-NLS-2$
			return null;
		}
	...
	private Map<Key, String> fDisabledProjectSettings; // null when project specific
	...
	// return getKey(Activator.ID_PLUGIN, key);
	// }
	//
	...
	private int getRebuildCount() {
		return fManager.getWorkingCopy(DefaultScope.INSTANCE.getNode(Activator.ID_PLUGIN)).getInt(REBUILD_COUNT_KEY, 0);
	}

	private void incrementRebuildCount() {
		fRebuildCount++;
		fManager.getWorkingCopy(DefaultScope.INSTANCE.getNode(Activator.ID_PLUGIN)).putInt(REBUILD_COUNT_KEY,
				fRebuildCount);
	}
	...
	private void addHighlight(final Composite parent, final Label labelControl, final Combo comboBox) {
		...
		MouseTrackAdapter labelComboListener = new MouseTrackAdapter() {
			...
			public void mouseExit(MouseEvent e) {
				if (!comboBox.isFocusControl()) highlight(parent, labelControl, comboBox, HIGHLIGHT_NONE);
			}
		};
		...
		class MouseMoveTrackListener extends MouseTrackAdapter implements MouseMoveListener, MouseListener {
			...
			@Override
			public void mouseDown(MouseEvent e) {
				if (isAroundLabel(e)) comboBox.setFocus();
			}
	...
	protected int getHighlight(Label labelControl) {
		Object data = labelControl.getData();
		if (data == null) return HIGHLIGHT_NONE;
	...
	/**
	 * Sets the option {@code key} to the value {@code value}. Note that callers have to
	 * make sure the corresponding controls are updated afterwards.
	 *
		...
		if (enable != hasProjectSpecificOption && fProject != null) {
			if (enable) {
				...
				for (Key curr : fAllKeys) {
					...
					curr.setStoredValue(fLookupOrder[0], null, fManager); // clear project
																			 // settings
				}
			}
		}
	...
	protected boolean processChanges(IWorkbenchPreferenceContainer container) {
		...
		if (container != null) {
			// no need to apply the changes to the original store: will be done by the
			// page container
	...
	/**
	 * Updates the UI from the current settings. Must be called whenever a setting has
	 * been changed by code.
	 */
	...
	protected Link getCheckBoxLink(Key key) {
		if (fCheckBoxes == null) return null;
